...
class Parser:
    ...
    def parse_for_statement(self) -> List[str]:
        ...
        unrolled_loop_program = []
        unrolling_factor = self.uf  # Unroll by 4 iterations
        for i in range(unrolling_factor):
            unrolled_loop_program += loop_program

        # increment_ins = ["%s += %d;" % (loop_end_assignment_program[0].split()[0], unrolling_factor)]  # Adjust step

        branch_ins = ["branch(%s);" % (loop_start_label)]  # Branch back
